        ...
        <div class="calender-head__navigation">
          ...
          <span class="date-label">
            ...
            <template
              ...
              "
              >{{ moment(moment().day('Sunday').year(year).week(currentWeek).toDate()).format('MMM') }}</template
            ><template v-else>{{
              moment(
                moment()
                  .day('Sunday')
...
const WeekCalendar = defineComponent({
  ...
  setup() {
    ...
    const maximumDate = () => {
      return parseInt(
        moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('DD'),
        10,
      ) +
        6 <=
        moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).daysInMonth()
        ? parseInt(moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('DD'), 10) +
            6
        : parseInt(moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('DD'), 10) +
            6 -
            parseInt(
    ...
    const daysOfMonth = moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).daysInMonth();
    ...
    const daysOfWeek = () => {
      ...
      if (minimumDate() + 6 <= daysOfMonth) {
        for (let i = minimumDate(); i <= maximumDate(); i += 1) {
          days.push(
            `${moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('MM')}/${i}/${
              year.value
            }`,
          );
        }
        ...
        for (let j = 1; j <= minimumDate() + 6 - daysOfMonth; j += 1) {
          days.push(
            `${
              parseInt(
                moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('MM'),
                10,
              ) < 12
                ? parseInt(
                    moment(moment().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('MM'),
