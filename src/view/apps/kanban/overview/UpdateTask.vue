  ...
  <sdModal class="sDash_task-details" :visible="modalVisible" :footer="null" :onCancel="handleCancel">
    ...
    <div class="sDash_task-details-modal">
      <div class="sDash_task-details-modal__description">
        ...
        <a-textarea name="task-details-label" placeholder="Add a more detailed description…" />
      </div>
    </div>
    <div v-if="isLoading"></div>
    <ChecklistWrap v-else>
      <div class="sDash_checklist-block">
        <div class="addChecklist-wrap">
          ...
          <div v-if="editable" class="addChecklist-form">
            <a-input
              ...
              @pressEnter="() => onCheckListSubmit(data.id, data.boardId, data.checklist)"
            />
            <div class="addChecklist-form-action">
              <sdButton
                @click="() => onCheckListSubmit(data.id, data.boardId, data.checklist)"
                class="btn-add"
                size="small"
                type="primary"
              >
        ...
        <div class="sDash_checklist-row">
          <div v-for="(item, i) in data.checklist" class="sDash_checklist-item" :key="i">
            <div class="sDash_checklist-item__top">
              <h4 class="sDash_checklist-item__title">{{ item.label }}</h4>
              <sdButton
                @click="(event) => onCheckListDelete(data.id, data.boardId, data.checklist, item.id, event)"
                class="btn-delete"
                type="light"
              >
                Delete
            ...
            <div class="sDash_checklist__progress">
              <a-progress
                v-if="item.checkListTask.length"
                :percent="
                  Math.round(
                    (100 * item.checkListTask.filter((checked) => checked.checked === true).length) /
              ...
              <ul class="sDash_checklist-tasks">
                <li v-for="(task, i) in item.checkListTask" class="sDash_checklist-tasks__single" :key="i">
                  ...
                  <sdDropdown :action="['click']" class="wide-dropdwon kanbanCard-more">
                    <template #overlay>
                      <a @click="() => onCheckListItemDelete(item.id, task.id)" href="#">
              ...
              <div class="addChecklistTask-wrap">
                ...
                <div v-else class="addChecklistTask-form">
                  <a-input
                    ...
                    placeholder="Checklist Title"
                    @pressEnter="() => onCheckListItemAdd(item.id, item.checkListTask)"
                  />
                  <div class="addChecklistTask-form-action">
                    <sdButton
                      @click="() => onCheckListItemAdd(item.id, item.checkListTask)"
                      class="btn-add"
                      size="small"
                      type="primary"
                    >
