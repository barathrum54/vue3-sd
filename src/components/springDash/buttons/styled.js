...
const transparents = (theme, type) => {
    return `
        ...
        &:hover, &:focus {
            background: ${type !== 'default' && theme[`${type}-hover`]}25 !important;
            border-width: 0px;
...
const ButtonStyled = Styled(Button, props)`
    ...
    background: ${({ type,transparent, theme }) => (type !== 'default' && !transparent ? theme[`${type}-color`] + ' !important;' : '')};
    border-width: 0px;
    ...
    padding: 0px 20.5px;
    height: ${({ size, theme }) => (size !== 'default' ? theme[`btn-height-${size}`] + "!important;" : '42px !important;')};
    font-weight: 500;
    box-shadow: 0 0;
    &:hover, &:focus {
        background: ${({ type, transparent, theme }) => (type !== 'default' && type !== 'white' && !transparent ? theme[`${type}-hover`] + '' : '')};
        color: ${({ type }) => (type !== 'default' && type !== 'white' ? '#ffffff' : '#5A5F7D')};
    }
    i,
    svg,
